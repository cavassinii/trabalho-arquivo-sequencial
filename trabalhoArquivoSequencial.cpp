//Trabalho sobre Arquivos Sequenciais - 03/06/2024
#include <iostream>
#include <conio.h>
#include <cstring>
#include <cstdlib>
...
int main(){
    ...
    instrutor instrutores[TAMAX];
    int qtdRegistrosInstrutor;

    leituraInstrutor(instrutores, qtdRegistrosInstrutor, cidades, qtdRegistrosCidade);
}*/


int buscaBinariaCidade(cidade vetorCidade[], int cod, int qtdCidades){
   /* int j = 0, f = qtdCidades - 1;
    int m = (j + f) / 2;
    for (; f >= j && m > 0 && cod != vetorCidade[m].codigo; m = (j + f) / 2){
...
bool buscaSerialInstrutor(instrutor vetorInstrutor[], int cod, int cont){
    ...
    /*int j = 0, f = cont - 1;
    int m = (j + f) / 2;
    for (; f >= j && m > 0 && cod != vetorInstrutor[m].codigo; m = (j + f) / 2){
